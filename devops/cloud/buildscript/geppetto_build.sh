...
APPLICATION='/ibmproject'

CUSTOMSERVICEPATH='../../../services/custom_services'

HELMPATH='../devops/cloud'

DESKTOPCODE='../../../application/client/desktop/ibmproject'
DESKTOPIMAGENAME='geppettotest/ibmproject-desktop:1.0'
...
helm install --name ibmproject ./helm
if [ $? -eq 0 ]; then
    echo "App Deployment is Done"
    export NODE_PORT=$(kubectl get --namespace ibmproject -o jsonpath="{.spec.ports[0].nodePort}" services ibmproject-system-entry)
    export NODE_IP=$(kubectl get nodes --namespace default -o jsonpath="{.items[0].status.addresses[1].address}")
    export LOGGING_PORT=$(kubectl get --namespace ibmproject-logging -o jsonpath="{.spec.ports[0].nodePort}" services kibana)
    echo "------------------------"
